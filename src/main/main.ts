...
let pendingResources: import('../lib/api').IngestResource[] | null = null
...
async function flush(): Promise<void> {
  ...
  try {
    const res = await ingestInventory(token, items, lastCurrencies, parts, progress, completed, syndicates, builds, resources)
    noteAuthSuccess()
...
async function pullInventory(gameId: number, api: NonNullable<typeof owApp.overwolf.packages.gep>): Promise<void> {
  ...
  try {
    ...
    const completed = extractCompletedTodos(wrapped)
    if (completed.length) {
